...
class ShapefileToParquet:
    ...
    def shapefiles_to_parquet(self, shapefile_paths: list):
        """
        Converts shapefiles to Parquet files using parallel processing.
            ...
            if 'geometry' in gdf.columns:
                try:
                    ...
                    for col in gdf.columns:
                        ...
                        if gdf[col].dtype == 'int64' or gdf[col].dtype == 'int32':
                            gdf[col] = gdf[col].astype('float32')
        ...
        # Parallel processing of shapefiles
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(process_shapefile, file) for file in shapefile_paths]
    ...
    def run(self, country_codes: list = []):
        ...
        if not shapefile_paths:
            ...
            return
        self.shapefiles_to_parquet(shapefile_paths)
...
def main():
    ...
    concatonator = ConcatonateParquet(parquet_dir, output_parquet, max_batch_size_mb=100)
    concatonator.make_batches()
    batch_files = concatonator.list_parquet_files()
    concatonator.concat_batch_parquet_files(batch_files)
