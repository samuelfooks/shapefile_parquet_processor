...
class ParquetConcatenator:
    ...
    def concat_parquets_in_chunks(self):
        """Concatenates Parquet files into multiple chunks based on chunk_size_mb."""
        parquet_files = [f for f in os.listdir(self.output_dir) if f.endswith('.parquet')]
        if not parquet_files:
        ...
        for path in parquet_paths:
            ...
            if current_chunk_size + file_size > chunk_size_bytes and current_chunk_size > 0:
    ...
    def concatenate_to_single_parquet(self, final_output_file: str, row_group_size_mb: int = 500):
        """
        Concatenates all Parquet files in the output directory into a single Parquet file
        with multiple internal row groups.

        :param final_output_file: Path to the final concatenated Parquet file.
        :param row_group_size_mb: Desired size of each row group in MB.
        """
        parquet_files = [f for f in os.listdir(self.output_dir) if f.endswith('.parquet')]
        if not parquet_files:
            self.logger.error('No Parquet files found to concatenate.')
        ...
        parquet_paths = [os.path.join(self.output_dir, f) for f in parquet_files]
        self.logger.info(f'Found {len(parquet_paths)} Parquet files for single-file concatenation.')

        # Read and concatenate tables
        try:
            tables = [pq.read_table(f) for f in parquet_paths]
            self.logger.info(f'Read {len(tables)} tables successfully.')
        except Exception as e:
            self.logger.error(f'Error reading Parquet files: {e}')
            return

        # Ensure all tables have the same schema
        try:
            base_schema = tables[0].schema
            for i, table in enumerate(tables[1:], start=2):
                if not table.schema.equals(base_schema):
                    self.logger.error(f"Schema mismatch detected in file {parquet_paths[i-1]}.")
                    return
            self.logger.info('All tables have consistent schemas.')
        except Exception as e:
            self.logger.error(f'Error checking table schemas: {e}')
            return

        # Concatenate all tables
        try:
            concatenated_table = pa.concat_tables(tables)
            self.logger.info('Successfully concatenated all tables.')
        except Exception as e:
            self.logger.error(f'Error concatenating tables: {e}')
            return

        # Determine rows per row group
        try:
            average_row_size = concatenated_table.nbytes / concatenated_table.num_rows
            rows_per_group = max(int((row_group_size_mb * 1024 * 1024) / average_row_size), 1)
            self.logger.info(f'Average row size: {average_row_size:.2f} bytes.')
            self.logger.info(f'Rows per row group: {rows_per_group}.')
        except Exception as e:
            self.logger.error(f'Error calculating row group size: {e}')
            return

        # Write to single Parquet file with multiple row groups
        try:
            writer = pq.ParquetWriter(
                final_output_file,
                concatenated_table.schema,
                compression='snappy'
            )
            for i in range(0, concatenated_table.num_rows, rows_per_group):
                batch = concatenated_table.slice(i, rows_per_group)
                writer.write_table(batch)
                self.logger.info(f'Wrote rows {i} to {i + rows_per_group}')
            writer.close()
            self.logger.info(f'Successfully wrote concatenated Parquet file to {final_output_file}')
        except Exception as e:
            self.logger.error(f'Error writing concatenated Parquet file: {e}')
...
class WorkflowManager:
    ...
    def run_step3_concatenate_parquets(self, final_output_file: str):
        self.logger.info("=== Step 3: Concatenating Parquet Files into a Single File ===")
        self.parquet_concatenator.concatenate_to_single_parquet(final_output_file=final_output_file, row_group_size_mb=500)
    ...
    def save_failed_files_log(self):
        ...
        failed_files_log_path = os.path.join(self.logger_instance.log_dir, 'failed_files.csv')
        failed_files_df.to_csv(failed_files_log_path, index=False)
        self.logger.info(f'Saved failed files log to {failed_files_log_path}')

    def run(self, steps: List[int], final_output_file: str):
        for step in steps:
            if step == 1:
                ...
                self.run_step3_concatenate_parquets(final_output_file=final_output_file)
            else:
...
def main():
    ...
    config_path = os.path.join(wkdir, 'config.json')  # Updated to use os.path.join for cross-platform compatibility
    try:
    ...
    final_output_file = config.get("final_output_file", "final_concatenated.parquet")  # Added to config
    ...
    final_output_path = os.path.join(workflow_manager.output_dir, final_output_file)
    workflow_manager.run(steps=steps, final_output_file=final_output_path)
