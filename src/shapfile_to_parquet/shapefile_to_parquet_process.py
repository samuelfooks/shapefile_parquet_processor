...
class FailedFileCorrector:
            ...
            if dtype_mismatches:
                ...
                for col, dtypes in dtype_mismatches.items():
                    try:
                        ...
                        df[col] = self.align_dtypes(df[col], dtypes['parquet_dtype'], dtypes['shapefile_dtype'])
                        
                        self.logger.info(f"Converted column '{col}' to {dtypes['parquet_dtype']}.")
                    except Exception as e:
    ...
    def align_dtypes(self, columndata, dtype_parquet, dtype_shapefile):
        """
        Aligns data types between a DataFrame and a dictionary of column data types.
        """
        if dtype_parquet == dtype_shapefile:
            self.logger.info(f"Column data type already matches Parquet: {dtype_parquet}")
            return columndata.astype(dtype_parquet)
        else:
            if dtype_shapefile == 'int32' or dtype_shapefile == 'int64':
                # convert to float32 and fill NaN
                return columndata.astype('float32')
            elif dtype_shapefile == 'float64':
                # convert to float32
                return columndata.astype('float32')
            elif dtype_shapefile == 'object':
                # convert to string
                return columndata.astype('str')
            else:
                self.logger.error(f"Unsupported data type: {dtype_shapefile}")
                return columndata
...
class WorkflowManager:
    def __init__(self, input_dir: str, output_dir: str, log_dir: str, file_name_selectors: List[str] = None):
        ...
        self.parquet_consolidator = ParquetConsolidator(output_dir, self.logger) 
...
if __name__ == '__main__':
    ...
    with LocalCluster(n_workers=2, threads_per_worker=2, memory_limit='5GiB') as cluster:
